...
const BeritaHome = ({ page, berita }: any) => {
    ...
    <>
      <div className={styles['container']}>
        ...
        <div className={styles['pagination']}>
          <Link href={`/berita?page=${1}`} className={styles['circle']}>
          ...
          <Link href={`/berita?page=${currentPage == 1 ? currentPage : currentPage - 1}`} className={styles['circle']}>
          ...
          </Link>
          {
            ...
            ?
              Array.from({length: 5}, (_, i) => i + counter).map((num, i) => {
                if (currentPage >= 4) {
                  if (i === 0)
                    return (<Link key={i} href={`/berita?page=1`} className={styles['circle']}>1</Link>)
                  if (i === 1)
                ...
                if (currentPage <= pageSize - 3) {
                  if (i === 3)
                    return (<div className={styles['square']}>...</div>)
                  if (i === 4)
                    return (<Link key={i} href={`/berita?page=${pageSize}`} className={styles['circle']}>{pageSize}</Link>)
                }
                if (num === currentPage)
                  return (<div key={i} className={styles['circle_selected']}>{num}</div>)
                return (<Link key={i} href={`/berita?page=${num}`} className={styles['circle']}>{num}</Link>)
              }) 
            :
              Array.from({length: pageSize}, (_, i) => i + 1).map((num, i) => {
                if (num === parseInt(page))
                  return (<Link key={i} href={`/berita?page=${num}`} className={styles['circle_selected']}>{num}</Link>)
                return (<Link key={i} href={`/berita?page=${num}`} className={styles['circle']}>{num}</Link>)
              })
          }
          <Link href={`/berita?page=${currentPage == pageSize ? currentPage : currentPage + 1}`} className={styles['circle']}>
            <svg width="27" height="27" viewBox="0 0 27 27" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M11.5919 7.125L10.0938 8.62312L14.96 13.5L10.0938 18.3769L11.5919 19.875L17.9669 13.5L11.5919 7.125Z" fill="black"/>
            </svg>
          </Link>
          <Link href={`/berita?page=${pageSize}`} className={styles['circle']}>
